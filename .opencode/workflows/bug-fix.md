...
- Validation confirms bug is resolved (with regression test)
- Fix deployed to affected environments

## Flow

1. **Reproduce** — Coder confirms the bug with exact reproduction steps
2. **Root cause** — Coder analyzes the underlying cause; document findings
3. **Security assessment** — If the bug involves auth, data, access control, or PII
4. **Implementation** — Write the fix with a test that covers the regression
5. **Validation** — Code review via reviewer; QA validates fix and runs regression suite
6. **Deployment** — Deploy via DevOps (accelerated path for critical/security bugs)
